...
(function () {
  ...
  var CATALOGS = {
    zh: {
      ...
      "why.3p": "agent 写源码 → 调用 <code>builder.build</code> → 调用 <code>core.spawn</code> → 新工具立即可用——被 catalog 拒绝的 spawn（工具名冲突；命名空间扁平且全局唯一）会让调用失败并给出原因，同时回滚该次尝试，名字立即可以重试。新增能力本身就是一次工具调用，LLM 可以在对话过程中自行完成整套流程。社区组件也以相同方式安装：<code>plugin_install</code> 会克隆源码、编译并启动；执行前需人工批准，并始终从发布的源码构建。<code>fabric</code> 更进一步：LLM 可以编写掌控整个回合内工具调用流程的 Nim guest 程序，由组件编译为本地进程运行。外部 MCP 服务器也接入同一条总线：<code>mcp</code> 管理器为每个服务器启动一个桥接进程，其工具以普通目录工具的形式出现。",
      "why.4h": "<span class=\"num\">04</span> 能力形态持久化",
      "why.4p": "能力可以跨重启保留：已启动的组件记录在 store 中，并在下次启动时自动恢复。仓库保存可复现的源码；<code>var/bin/</code> 可以重新构建，持久化状态保存在 store 引擎中（默认 SQLite 的 <code>var/store.db</code>，可通过 <code>NIF_STORE_BACKEND</code> 换用 barrel 或 TiDB）。比总线中断存活更久的组件会自行重新接入：重新解析总线、重建订阅并重新宣告，无需结束进程。",
      "arch.title": "架构",
      ...
      "qs.install": "# 将 niffler / niffler-cli / niffler-console 放入 PATH（UI 插件装好后会一并链接 niffler-ui；按需加装 niffler-tui）",
      ...
      "qs.see": "└─ make run / recover / down / down-here / ram / uninstall · 见 docs/MANUAL.md",
      ...
      "comp.th.language": "语言",
      "comp.th.purpose": "用途",
      "comp.core.p": "对话循环、进程管理（supervisor）、组件目录（catalog）和调用分发（dispatch）——以及每次 provider 请求前的上下文窗口准入守卫；属于人类的两个对话级控件通过 <code>session</code> 调用设置，并以 <code>/approvals</code>（本对话的门禁模式：<code>ask</code> 或 <code>auto</code>，auto 会高声自动放行）与 <code>/limit</code>（软性 rounds/tokens/seconds 预算，触及时询问“是否继续”而非结束回合）暴露，随对话持久化；回合进行中到达的 session 调用会立即以 <code>busy</code> 拒绝，不再悬挂——审批等待期间也不例外，因为等待循环会泵送与工具分发相同的空闲表面，而停止指令（<code>__cancel</code>）会以拒绝方式中断等待；<code>session {export: true}</code> 导出下一回合将要发送的确切请求（消息、经允许清单过滤的工具 schema、模型、provider、<code>reasoning_effort</code>），不产生用户消息、LLM 调用或 store 写入；超时或被取消的工具调用会带着部分输出以显式错误返回，而不是被记成静默的成功",
      "comp.session.p": "一个对话 = 一个进程——临时按会话启动的 runner，从 store 恢复；杀掉一个只会丢失进行中的回合",
      "comp.bash.p": "以工具形式提供 shell 访问；执行前需人工批准；支持进程树取消与超大输出分页读取",
      "comp.builder.p": "将 agent 编写的 Nim/Go/TypeScript 源码编译为可执行程序，并用 <code>build_package</code> 构建 manifest-v2 插件项目——依赖由源码声明，而非构建参数：Go 构建用 <code>go mod tidy</code> 解析导入，TS 构建在基础安装之后扫描入口的导入并 npm 安装外部包（解析出的版本范围写入生成的 <code>package.json</code>，回复中以 <code>deps</code> 列出）；插件项目的依赖清单与锁文件由包自己持有，builder 负责暂存项目、只展开受控的 SDK/输出占位符、执行有界 argv 配方（不允许 shell 包装），并校验声明的产物后再发布",
      "comp.store.p": "总线文档存储，使用版本号（rev）进行乐观并发控制——同一契约下引擎可互换：SQLite（默认，<code>var/store.db</code>）、barrel 与 TiDB/MySQL，通过 <code>NIF_STORE_BACKEND</code> 选择；<code>list</code> 是分页读取（单页上限 1000 条），返回 <code>hasMore</code> 与 <code>nextAfter</code> 游标，core 自身的全量读取会自动分页；<code>search {kind, query, limit?, after?}</code> 是服务端过滤——按会话标题/消息正文的大小写无关词前缀匹配，SQLite 由 FTS5 索引作答，其他引擎以同一匹配器按 id 顺序扫描；存在未迁移 <code>var/barrel-db</code> 历史的 harness 会拒绝启动——<code>niffler-store-migrate</code> 可离线在任意引擎对之间迁移根目录（<code>--scan</code>/<code>--all</code>/<code>--dry-run</code>）",
      "comp.plugins.p": "组件生态：按 topic 搜索，通过 <code>niffler.json</code> 描述包——manifest v1 的紧凑形式，或 manifest v2（<code>manifestVersion: 2</code>）项目：由包自带依赖清单与锁文件、经 builder 的 <code>build_package</code> 配方构建；TypeScript 包也能安装（manifest 读取器接受 <code>lang: \"ts\"</code>），条目像其他组件一样启动——支持安装/更新/移除，始终从源码构建；提供按组件名加前缀的 <code>/plugins</code>/<code>/plugins-search</code>/<code>/plugins-install</code>/<code>/plugins-update</code>/<code>/plugins-remove</code> 斜杠命令；仓库没有发布 release 时标签解析会回退到它的 tags，分支钉选的安装会跟随分支原地更新、不会被改指到 release 标签，而无变化的更新也会在已安装产物过期或缺失时（二进制被删、源码被改过）就地重建——重建失败时保留旧组件继续运行",
      ...
      "comp.recall.p": "按需的 <code>context_recall</code> 解析器——被修剪和溢出的内容以引用替代，本组件负责解析它们：canonical 消息、晋升的溢出文档和当前持久的 compaction 检查点；每条修剪/溢出通知都逐字给出引用名，<code>mode: search</code> 则在整个 canonical 历史（含已裁剪/已压缩的消息）中检索提到某内容的消息。按需加载、绝不隐藏：模型经 discover + invoke 触达它，通知里的建议因此真正可执行",
      "comp.compaction.p": "可替换的上下文摘要器——长回合能在自己的上下文中存活：每次 provider 请求前 core 都会按 provider 自己的 token 尺度估算请求规模，在压力线执行确定性阶梯（无损工具结果修剪 → 摘要压缩 → 整回合裁剪 → 显式 <code>context-recovery-required</code>），绝不发出超出窗口的请求；持久化裁剪会记录水位线，重放可跨重启安全进行，归一化后的 provider 溢出 400 会先获得一次带回执的恢复尝试再转为终态。本组件是默认的 contract-v1 <code>compaction_propose</code> 实现（<code>NIF_COMPACTION_TOOL</code> 可选择其他实现）：它校验 runner 的分页快照、选择允许的裁剪点并起草检查点候选——只有 runner 会验证并提交 <code>context_projection</code>，canonical 消息保持不可变；任何 contract-v1 实现都可用 <code>make test-conformance</code> 对照契约验证；所提供的裁剪点集合与提交守卫已对齐，因此先裁剪后压缩的对话也能再次压缩（省略通知的边界会解析为检查点实际吸收的第一条 canonical 条目）；手动 <code>/compact</code> 会报告精确的拒绝/失败原因，而非笼统的“无可裁剪”，每一次非 reset 退出都会送达总线（<code>compact:unavailable|failed|declined|invalid|stale</code>，各带可读的 <code>detail</code>），辅助摘要继承对话已解析的 provider/model，不跟随之后的全局切换",
      ...
      "comp.repomap.p": "排序后的仓库地图（aider repomap 的移植）——以约 1KB 的篇幅给出最关键的文件及其关键定义，由 tree-sitter + 原生 Nim 的 tags 图经个性化 PageRank 排序得出；<code>repo_map</code> 是 onDemand 且只读的工具，不受附加门禁限制。工作区打开时的自动附加（每个新对话一条只追加条目）默认开启但设有门禁：低于普查下限（<code>NIF_REPOMAP_MIN_CENSUS</code>，默认 50 个覆盖文件）的工作区不建图，骨架图（字节/符号/文件阈值）不注入，被拦截的地图会留下日志；<code>NIF_REPOMAP_AUTOAPPEND=0</code> 可关闭附加。缓存位于 <code>var/repomap-tags/</code>；组件缺席只意味着没有地图，别无影响",
      ...
      "comp.llm.p": "流式聊天 adapter——默认 OpenAI 兼容 Chat Completions（DeepSeek），另支持 OpenAI Codex（ChatGPT OAuth）Responses 与 Anthropic Messages 协议——实时 <code>ev.llm.token</code> token 流、推理 token，以及单次调用取消；回合的图像附件落地为各协议的多模态 content 部分（OpenAI <code>image_url</code> data-URL、Anthropic <code>source</code> 块、Codex <code>input_image</code> 项）；重试按故障类型拆分预算——带 <code>Retry-After</code> 的 429 按服务器指示等待（封顶、不限尝试次数），流超时与连接拒绝各有独立的有界预算（<code>NIF_LLM_MAX_STREAM_RETRIES</code>/<code>NIF_LLM_MAX_CONNECT_RETRIES</code>，默认 2），永久性错误（鉴权、配额、错误请求）快速失败",
      ...
      "comp.agent.p": "subagent 会话——同步 <code>agent_run</code> 把任务委派给拥有全新上下文的子 runner；持久化后台任务（<code>agent_spawn</code>/<code>status</code>/<code>wait</code>/<code>stop</code>/<code>steer</code>）跨重启存续；子会话是有记忆的会话：<code>agent_run</code>/<code>agent_spawn {session}</code> 让既有子会话再跑一回合（只追加历史、基于持久血缘关系的授权、busy 与排队两种语义、激活账本、<code>close: true</code> 退役），<code>fork: true | {lastK} | {maxChars}</code> 用调用方的已完成回合播种一个全新子会话；未显式指定 <code>model</code> 的新子会话继承父对话的生效模型及其 provider 钉选，也可用 <code>modelTier: weak | medium | strong</code>（会被钳制到父对话的档位）；委派深度有上限（<code>NIF_AGENT_MAX_DEPTH</code>，默认 1——子代理不能再派生子代理），嵌套调用使用按 id 键控、带独立截止时间的租约，并行的会话上下文分发因此安全；已结束的后台子会话以结算通知送达父对话（是指向 <code>agent_status</code> 所返回完整回复的指针），无需轮询；<code>agent_list</code> 给出派生的子会话名册，对空闲子会话的 steer 会排入持久的父邮件并在其下一回合送达，<code>agent_ask</code> 返回子会话的回答；空闲 runner 自动退役",
      ...
      "comp.nats.p": "总线服务器本身，从源码构建——components/nats 将官方 nats-server 编译为 <code>var/bin/nats-server</code>；core 优先使用该二进制而非 PATH 安装，因此仅 <code>make build</code> 即可满足总线依赖；内置构建以 8 MiB 载荷上限运行（上游默认 1 MiB），携带完整对话的 LLM 请求也放得下；回退到 PATH 安装时经配置文件取得同样的 8 MiB 上限（core 回退时会警告一次），core 还会在启动时检查所连总线的最大载荷，发现不够就大声警告而不是在对话中途失败；SDK 同样感知中断——两者都会探测连接健康，超过 <code>NIF_RECONNECT_GRACE_S</code>（默认 180 秒）仍不健康时重新接入（重解析总线、重拨、重建所有订阅、重新发布 <code>reg.publish</code>），而不是活着却失聪",
      ...
      "status.l73": "plugins 斜杠命令 + 更新语义——<code>/plugins</code>/<code>/plugins-search</code>/<code>/plugins-install</code>/<code>/plugins-update</code>/<code>/plugins-remove</code> 按组件名加前缀；仓库没有 release 时标签解析回退到 tags，分支钉选的安装跟随分支更新，不会被改指到 release 标签，而无变化的更新也会在已安装产物过期或缺失时就地重建",
      ...
      "status.l81": "上下文压缩——每次 provider 请求前先做准入，配合确定性阶梯（无损修剪 → 摘要压缩 → 整回合裁剪 → 显式 <code>context-recovery-required</code>）、可替换的 contract-v1 压缩器（<code>NIF_COMPACTION_TOOL</code>）、不可变的 canonical 消息与 <code>context_recall</code> 引用、可归因的 <code>reset:*</code> 前缀重建，以及能证明任意 contract-v1 实现的 <code>make test-conformance</code> 运行器；每一次压缩拒绝都会送达总线（<code>compact:unavailable|failed|declined|invalid|stale</code>，各带可读的 <code>detail</code>），先裁剪后压缩的对话也能再次压缩——所提供的裁剪点集合与提交守卫已对齐",
      "status.l82": "subagent 续聊 + fork——<code>agent_run</code>/<code>agent_spawn {session}</code> 让既有子会话再跑一回合（只追加历史、基于持久血缘关系的授权、busy 与排队两种语义、激活账本、<code>close: true</code> 退役）；<code>fork</code> 用调用方的已完成回合播种全新子会话——从 0 起连续、均衡的裁剪点，失败即拒绝，出生时缓存为冷；未显式指定 <code>model</code> 的新子会话继承父对话的生效模型及其 provider 钉选",
      ...
      "status.l95": "排序仓库地图（repomap 组件）——aider 风格的工作区地图，由 tree-sitter + 原生 Nim 的 tags 图经个性化 PageRank 排序得出；<code>repo_map</code> 是 onDemand 且只读（模型主动询问，不受门禁限制），工作区打开时的自动附加默认开启但设有准入门禁——低于普查下限（<code>NIF_REPOMAP_MIN_CENSUS</code>，默认 50 个覆盖文件）的工作区不建图，骨架图（字节/符号/文件阈值）不发布，被拦截的地图会留下日志；<code>NIF_REPOMAP_AUTOAPPEND=0</code> 可关闭（<code>bench/reports/repomap-gates-full30.md</code> 验证门禁会拦截微型仓库与骨架图，<code>bench/reports/repomap-ab-*.md</code> 是随场景而异的 A/B 历史）",
      ...
      "status.l100": "并发回合分发 + 每 session 事件主题——core 绝不阻塞在 runner 上：每个 session 调用（含开启回合的调用）都走自己的私有转发收件箱，不同对话可以并发推进，而每个 runner 只串行化自己的对话（回合中的调用立即以 <code>busy</code> 拒绝）；runner 改发每 session 主题 <code>ev.session.&lt;id&gt;.&lt;kind&gt;</code>——盯住一个对话的客户端订阅 <code>ev.session.&lt;id&gt;.&gt;</code>，通配符可组合（<code>ev.session.*.token</code>），hooks 按 NATS 语义匹配主题",
      "status.l101": "TS 组件自行解析依赖——builder 用 TypeScript 自己的 <code>preProcessFile</code> 扫描入口导入并 npm 安装找到的外部包（跳过相对路径与 <code>node:</code> 内建模块，≤32 个，名称先经白名单），解析出的版本范围写入生成的 <code>package.json</code>，并在回复中以 <code>deps</code> 列出；plugins manifest 接受 <code>lang: \"ts\"</code>，TS 插件包从此可经 <code>plugin_install</code> 端到端安装",
      "status.l102": "审批等待保持响应——回合停在审批提示时，等待循环会泵送与工具分发相同的空闲表面：等待期间到达的 session 调用立即得到 <code>busy</code> 拒绝，而不是沉默直到 5 分钟超时；steer 保持可用；停止指令（<code>__cancel</code>）以拒绝方式中断等待，回合以取消结束而不是等满审批超时；嵌套的审批问题会被立即拒绝——人类一次只回答一个弹窗",
      "status.l103": "双通道取消让停止真正落地——停止回合也会中止并行的工具调用波（剩余的每个调用各自收到一次 <code>cancel.&lt;component&gt;</code> 发布，并保留部分回复的常规宽限，与单次分发一致），LLM 重试退避改为约 250 ms 切片泵送，停止在四分之一秒内生效而不是排在整段延迟之后",
      "status.l104": "手动 <code>/compact</code> 的上下文量表——提交会自行发布一条状态帧（reason <code>reset:compact</code>、generation、beforeTokens/afterTokens、usedTokens = 标记 <code>estimated: true</code> 的本地估算），量表立即反映压缩结果；下一次请求的实测用量会替换该估算，被拒绝的压缩不发布任何帧，UI 会提示所示数值为估算值（三种语言均已本地化）",
      "status.l105": "清理失效的交互客户端——系统目录每 30 秒按 pid 存活检查客户端并剔除死条目（<code>lost (client process gone)</code>——硬退出的 TUI、关闭的终端、<code>/restart</code> 的后继者），让 <code>clientCount</code> 与自动启动 core 的空闲关停保持真实；pid 复用可能短暂保留陈旧条目，ui 注册表的租约仍是对话所有权的权威",
      "status.l106": "桌面 UI 变为插件——<code>ui/</code> 源码已迁至 <code>gokr/niffler-ui</code> 仓库；<code>make install-ui</code> 会启动一个隔离的自动批准 harness，并经插件管理器安装 UI（按最新 release 标签克隆、针对本 harness 构建、发布为 <code>var/bin/niffler-ui</code>），<code>make build</code> 只构建 core + 组件，前端测试也迁入 UI 自己的仓库——<code>make test</code> 重新成为纯粹的总线契约套件",
      "status.l107": "manifest v2 包构建——包可声明 <code>{manifestVersion: 2, components: [{name, lang, project, build: {steps, artifact}}]}</code> 并自带依赖清单与锁文件（<code>package.json</code>/锁文件、<code>go.mod</code>/<code>go.sum</code>、Nimble）；builder 的 <code>build_package</code> 暂存项目、只展开受控的 SDK/输出占位符、执行有界 argv 配方（工具链可以组合——例如 <code>npm ci</code> 后接 <code>wails build</code>——但不允许 shell 包装），并在发布前校验声明的产物",
      "status.l108": "小组件的自愿自检——<code>grep</code>、<code>logfile</code>、<code>observe</code> 与 <code>hooks</code> 注册了 WIRE.md 描述的隐藏 <code>selftest</code>：快速接线检查，外加 deep 探针——真实的有界 rg 搜索、写入真实日志汇、演练 hooks 去重环、经 stdin 管道送入临时文件",
      "status.l109": "<code>core.spawn</code> 等到注册完成才回答——它会等待（<code>NIF_SPAWN_WAIT_MS</code>，默认 5000 毫秒，钳制在 250–120000），并在被拒绝（工具名冲突——命名空间扁平且全局唯一）或超时时让调用失败，错误携带 catalog 的拒绝原因或超时说明外加 <code>var/logs/&lt;name&gt;.log</code> 的有界尾段；该次尝试会被回滚（停止副本、删除 catalog 条目、不落库），名字立即可以重试，<code>cli install</code> 也会点明 plugin_install 返回的原因",
      "status.l110": "provider 与模型一并钉选——只带 <code>model</code> 的 <code>session</code> 调用会解析该模型所属的 provider 并在一次头部写入中同时持久化两者，遗留的「半钉选」在 runner 首次加载时自动修复，新子会话在继承模型的同时也继承 provider；<code>llm_resolve</code> 失败时 TUI 头部会标记过期并重试，而不是把上次的已知状态当作生效状态展示",
      "status.l111": "edit 对注册表已知的语言绝不静默——每次成功编辑后异步排队一次 lsp 诊断检查，编辑结果会点明该通道，或说明检查为何没有运行（文件在对话工作区之外、lsp 不可达）；只有扩展名不被注册表认领的文件才保持沉默",
      "status.l112": "基准 dsh 通道——DeepSeek Harness 加入 harness 基准（可选 <code>--harness dsh</code>，走其 sdk-minimal JSON-RPC 档）；直连 DeepSeek（无网关、一方 API）的全新 full30 配对在 thinking=low 双双 30/30，niffler 平均回合更少（6.0 对 6.8）、约 17 秒对 45 秒、39k 对 51k token（<code>bench/reports/full30-direct-dsh-rc3-low-report.md</code>）",
      "status.l113": "store 的 <code>search</code>——第五个 store 工具，各引擎契约一致：<code>search {kind, query, limit?, after?}</code> 返回与 <code>list</code> 完全相同的形状与排序（id 升序、同样的排他游标、limit 默认 100 / 上限 1000），会话浏览器因此在数据所在处过滤，无需把整个 <code>conversation</code> 类型下载后再本地匹配；索引字段按类型记录（<code>conversation</code> = id + 标题，<code>message</code> = id + <code>content</code> 下的各字符串（上限 16KB），其他类型 = 仅 id），查询与文本按字母/数字连续段分词，每个查询词都必须是某个已索引词的大小写不敏感前缀（AND），因此没有任何需要转义的内容；<code>store-sqlite</code>（默认）由 FTS5 索引作答（<code>docs_fts</code>，rowid 与 <code>docs</code> 共用，与文档在同一事务内维护，启动时若两者不一致即由 <code>docs</code> 重建——派生状态，丢弃无损），<code>barrel</code> 与 <code>store-tidb</code> 则按 id 顺序用同一匹配器扫描该类型",
      "status.l114": "回合中的图像（多模态）——session 调用接受 <code>attachments</code>（<code>{type: \"image\", name?, mimeType, data(base64), width?, height?}</code>）：core 校验每一项声明（base64 可解码、魔数与 PNG/JPEG/GIF/WebP/BMP 允许清单一致、每图 ≤4 MB、每回合 ≤8 张、每回合 ≤4.5 MB），没有文字说明的拖入同样会运行一个回合；请求落地为 OpenAI 多模态 content 数组、Anthropic 的 base64 <code>source</code> 块与 Codex 的 <code>input_image</code> 项——三种协议都能用图，输出计费按像素面积而非 base64 长度计价；像素与消息分开存储——每张图一个不含像素的 <code>attachment</code> 元数据文档加一个 <code>attachmentdata</code> 文档，因为把截图内联进 <code>list</code> 页会超出总线载荷上限——消息只保留引用，投影在 <code>NIF_ATTACH_BUDGET</code> 内保留最新的图像（更早的降级为诚实的文字标记），规则是引用的纯函数，因此重启会重建完全相同的请求，<code>conversation_delete</code> 会随对话一并清扫像素",
      "status.l115": "总线中断后的重新接入——比 NATS 客户端自身重连预算更久的中断不会再让组件“活着却失聪”：两个 SDK 都会探测连接健康（每 2 秒一次 Flush），超过 <code>NIF_RECONNECT_GRACE_S</code>（默认 180 秒，刻意高于客户端预算）仍不健康时，重新解析总线（<code>NIF_NATS_URL</code> → <code>var/nats-url</code> → 众所周知的端口，因此重启后换了端口的新 core 也能找到）、带回退地重拨、重建所有订阅并重新发布 <code>reg.publish</code>——无需重启进程；延迟宣告的组件（<code>mcp-bridge</code>）通过 <code>onReattached</code> 钩子重新宣告",
      "status.l116": "上下文计量会预警而不是漂移——被固定的 provider/model 组合若只解析出回退窗口（目录中没有该模型），状态帧会带上 <code>warning</code>，点名该组合与实际获得的窗口；<code>warn:threshold</code> 上下文事件带上 <code>trimAt</code>（以 token 计的有效档位），客户端因此能使用 core 打印的同一百分比；终端客户端会渲染每一种上下文原因——包括压缩拒绝——而不只是裁剪计数",
      ...
      "status.l57": "<code>make install</code> / <code>make uninstall</code>——将 niffler、niffler-cli、niffler-console 放入 PATH（UI 插件构建出 <code>var/bin/niffler-ui</code> 后会一并链接；按需加装 niffler-tui 包装器，<code>WITH_TUI=1</code>）；只安装 niffler 前缀的命令，绝不安装组件二进制，因此 PATH 不会遮蔽 Unix 工具",
      ...
      "status.t3": "store 的向量记忆——嵌入与相似度召回（全文检索已落地：SQLite 上的 FTS5 索引，各引擎以扫描提供同一匹配器）",
    ...
    "zh-TW": {
      ...
      "why.3p": "agent 寫原始碼 → 呼叫 <code>builder.build</code> → 呼叫 <code>core.spawn</code> → 新工具立即可用——被 catalog 拒絕的 spawn（工具名衝突；命名空間扁平且全域唯一）會讓呼叫失敗並給出原因，同時回滾該次嘗試，名稱立即可以重試。新增能力本身就是一次工具呼叫，LLM 可以在對話過程中自行完成整套流程。社群組件也以相同方式安裝：<code>plugin_install</code> 會複製原始碼、編譯並啟動；執行前需經人工確認，且一律從發布的原始碼建置。<code>fabric</code> 更進一步：LLM 可以撰寫掌控整個回合內工具呼叫流程的 Nim guest 程式，由元件編譯為本機行程執行。外部 MCP 伺服器也接入同一條匯流排：<code>mcp</code> 管理器為每個伺服器啟動一個橋接行程，其工具以普通目錄工具的形式出現。",
      "why.4h": "<span class=\"num\">04</span> 能力形態持久化",
      "why.4p": "能力可以跨重啟保留：已啟動的組件記錄在 store 中，並在下次啟動時自動復原。倉庫保存可重現的原始碼；<code>var/bin/</code> 可以重新建置，持久化狀態保存在 store 引擎中（預設 SQLite 的 <code>var/store.db</code>，可透過 <code>NIF_STORE_BACKEND</code> 改用 barrel 或 TiDB）。比匯流排中斷存活更久的組件會自行重新接入：重新解析匯流排、重建訂閱並重新宣告，無需結束程序。",
      "arch.title": "架構",
      ...
      "qs.install": "# 將 niffler / niffler-cli / niffler-console 放入 PATH（UI 外掛裝好後會一併連結 niffler-ui；視需求加裝 niffler-tui）",
      ...
      "qs.see": "└─ make run / recover / down / down-here / ram / uninstall · 見 docs/MANUAL.md",
      ...
      "comp.builder.p": "將 agent 撰寫的 Nim/Go/TypeScript 原始碼編譯為執行檔，並以 <code>build_package</code> 建置 manifest-v2 外掛專案——相依由原始碼宣告，而非建置參數：Go 建置以 <code>go mod tidy</code> 解析匯入，TS 建置在基礎安裝之後掃描入口的匯入並 npm 安裝外部套件（解析出的版本範圍寫入產生的 <code>package.json</code>，回覆中以 <code>deps</code> 列出）；外掛專案的相依清單與鎖檔由套件自持，builder 負責暫存專案、只展開受控的 SDK/輸出佔位符、執行有界 argv 配方（不允許 shell 包裝），並在發布前驗證宣告的產物",
      "comp.store.p": "匯流排文件儲存，使用版本號（rev）進行樂觀並行控制——同一契約下引擎可互換：SQLite（預設，<code>var/store.db</code>）、barrel 與 TiDB/MySQL，透過 <code>NIF_STORE_BACKEND</code> 選擇；<code>list</code> 是分頁讀取（單頁上限 1000 筆），回傳 <code>hasMore</code> 與 <code>nextAfter</code> 游標，core 自身的全量讀取會自動分頁；<code>search {kind, query, limit?, after?}</code> 是伺服器端過濾——依工作階段標題/訊息正文的大小寫無關詞前綴比對，SQLite 由 FTS5 索引作答，其他引擎以同一比對器依 id 順序掃描；存在未遷移 <code>var/barrel-db</code> 歷史的 harness 會拒絕啟動——<code>niffler-store-migrate</code> 可離線在任意引擎對之間遷移根目錄（<code>--scan</code>/<code>--all</code>/<code>--dry-run</code>）",
      "comp.plugins.p": "組件生態：依 topic 搜尋，透過 <code>niffler.json</code> 描述套件——manifest v1 的緊湊形式，或 manifest v2（<code>manifestVersion: 2</code>）專案：由套件自帶相依清單與鎖檔、經 builder 的 <code>build_package</code> 配方建置；TypeScript 套件也能安裝（manifest 讀取器接受 <code>lang: \"ts\"</code>），條目像其他組件一樣啟動——支援安裝/更新/移除，一律從原始碼建置；提供依組件名加前綴的 <code>/plugins</code>/<code>/plugins-search</code>/<code>/plugins-install</code>/<code>/plugins-update</code>/<code>/plugins-remove</code> 斜線命令；倉庫沒有發布 release 時標籤解析會回退到它的 tags，分支釘選的安裝會沿分支原地更新、不會被改指到 release 標籤，而無變更的更新也會在已安裝產物過期或缺失時（二進位被刪、原始碼被改過）就地重建——重建失敗時保留舊組件繼續執行",
      ...
      "comp.recall.p": "按需的 <code>context_recall</code> 解析器——被修剪與溢出的內容以參照替代，本組件負責解析它們：canonical 訊息、晉升的溢出文件與目前持久的 compaction 檢查點；每條修剪/溢出通知都逐字給出參照名，<code>mode: search</code> 則在整個 canonical 歷史（含已裁剪/已壓縮的訊息）中檢索提到某內容的訊息。按需載入、絕不隱藏：模型經 discover + invoke 觸達它，通知裡的建議因此真正可執行",
      "comp.compaction.p": "可替換的上下文摘要器——長回合能在自己的上下文中存活：每次 provider 請求前 core 都會按 provider 自己的 token 尺度估算請求規模，在壓力線執行確定性階梯（無損工具結果修剪 → 摘要壓縮 → 整回合裁剪 → 明確的 <code>context-recovery-required</code>），絕不送出超出視窗的請求；持久化裁剪會記錄水位線，重放可跨重啟安全進行，正規化後的 provider 溢位 400 會先獲得一次帶回執的復原嘗試再轉為終態。本組件是預設的 contract-v1 <code>compaction_propose</code> 實作（<code>NIF_COMPACTION_TOOL</code> 可選擇其他實作）：它校驗 runner 的分頁快照、選擇允許的裁剪點並起草檢查點候選——只有 runner 會驗證並提交 <code>context_projection</code>，canonical 訊息保持不可變；任何 contract-v1 實作都可用 <code>make test-conformance</code> 對照契約驗證；所提供的裁剪點集合與提交守衛已對齊，因此先裁剪後壓縮的對話也能再次壓縮（省略通知的邊界會解析為檢查點實際吸收的第一條 canonical 條目）；手動 <code>/compact</code> 會報告精確的拒絕/失敗原因，而非籠統的「無可裁剪」，每一次非 reset 退出都會送達匯流排（<code>compact:unavailable|failed|declined|invalid|stale</code>，各帶可讀的 <code>detail</code>），輔助摘要繼承對話已解析的 provider/model，不跟隨之後的全域切換",
      ...
      "comp.repomap.p": "排序後的倉庫地圖（aider repomap 的移植）——以約 1KB 的篇幅給出最關鍵的檔案及其關鍵定義，由 tree-sitter + 原生 Nim 的 tags 圖經個人化 PageRank 排序得出；<code>repo_map</code> 是 onDemand 且唯讀的工具：模型主動詢問，不受附加門檻限制。工作區開啟時的自動附加（每個新對話一筆僅附加條目）預設開啟但設有門檻：低於普查下限（<code>NIF_REPOMAP_MIN_CENSUS</code>，預設 50 個涵蓋檔案）的工作區不建圖，骨架地圖（位元組/符號/檔案門檻）不注入，被攔截的地圖會留下日誌；<code>NIF_REPOMAP_AUTOAPPEND=0</code> 可關閉附加。快取位於 <code>var/repomap-tags/</code>；組件缺席只意味著沒有地圖，別無影響",
      ...
      "comp.llm.p": "串流聊天 adapter——預設 OpenAI 相容 Chat Completions（DeepSeek），另支援 OpenAI Codex（ChatGPT OAuth）Responses 與 Anthropic Messages 協定——即時 <code>ev.llm.token</code> token 串流、推理 token，以及單次呼叫取消；回合的影像附件落地為各協定的多模態 content 部分（OpenAI <code>image_url</code> data-URL、Anthropic <code>source</code> 區塊、Codex <code>input_image</code> 項）；重試按故障類型拆分預算——帶 <code>Retry-After</code> 的 429 按伺服器指示等待（封頂、不限嘗試次數），串流逾時與連線拒絕各有獨立的有界預算（<code>NIF_LLM_MAX_STREAM_RETRIES</code>/<code>NIF_LLM_MAX_CONNECT_RETRIES</code>，預設 2），永久性錯誤（鑑權、配額、錯誤請求）快速失敗",
      ...
      "comp.agent.p": "subagent 對話——同步 <code>agent_run</code> 把任務委派給擁有全新脈絡的子 runner；持久化背景任務（<code>agent_spawn</code>/<code>status</code>/<code>wait</code>/<code>stop</code>/<code>steer</code>）跨重啟存續；子對話是有記憶的對話：<code>agent_run</code>/<code>agent_spawn {session}</code> 讓既有子對話再跑一回合（僅附加歷史、基於持久血緣關係的授權、busy 與排隊兩種語義、激活帳本、<code>close: true</code> 退役），<code>fork: true | {lastK} | {maxChars}</code> 用呼叫方的已完成回合播種一個全新子對話；未明確指定 <code>model</code> 的新子對話繼承父對話的生效模型及其 provider 釘選，也可用 <code>modelTier: weak | medium | strong</code>（會被鉗制到父對話的層級）；委派深度有上限（<code>NIF_AGENT_MAX_DEPTH</code>，預設 1——子代理不能再派生子代理），巢狀呼叫使用按 id 鍵控、帶獨立期限的租約，並行的對話脈絡分派因此安全；已結束的背景子對話以結算通知送達父對話（是指向 <code>agent_status</code> 所回傳完整回覆的指標），無需輪詢；<code>agent_list</code> 給出派生的子對話名冊，對閒置子對話的 steer 會排入持久的父郵件並在其下一回合送達，<code>agent_ask</code> 回傳子對話的回答；閒置 runner 自動退役",
      ...
      "comp.nats.p": "匯流排伺服器本身，從原始碼建置——components/nats 將官方 nats-server 編譯為 <code>var/bin/nats-server</code>；core 優先使用該執行檔而非 PATH 安裝，因此僅 <code>make build</code> 即可滿足匯流排依賴；內建建置以 8 MiB 承載上限執行（上游預設 1 MiB），攜帶完整對話的 LLM 請求也放得下；回退到 PATH 安裝時經設定檔取得同樣的 8 MiB 上限（core 回退時會警告一次），core 還會在啟動時檢查所連匯流排的最大承載，發現不足就明確警告而不是在對話中途失敗；SDK 同樣感知中斷——兩者都會探測連線健康，超過 <code>NIF_RECONNECT_GRACE_S</code>（預設 180 秒）仍不健康時重新接入（重新解析匯流排、重撥、重建所有訂閱、重新發布 <code>reg.publish</code>），而不是活著卻失聰",
      ...
      "status.l73": "plugins 斜線命令 + 更新語義——<code>/plugins</code>/<code>/plugins-search</code>/<code>/plugins-install</code>/<code>/plugins-update</code>/<code>/plugins-remove</code> 依組件名加前綴；倉庫沒有 release 時標籤解析回退到 tags，分支釘選的安裝沿分支更新，不會被改指到 release 標籤，而無變更的更新也會在已安裝產物過期或缺失時就地重建",
      ...
      "status.l81": "上下文壓縮——每次 provider 請求前先做准入，搭配確定性階梯（無損修剪 → 摘要壓縮 → 整回合裁剪 → 明確的 <code>context-recovery-required</code>）、可替換的 contract-v1 壓縮器（<code>NIF_COMPACTION_TOOL</code>）、不可變的 canonical 訊息與 <code>context_recall</code> 參照、可歸因的 <code>reset:*</code> 前綴重建，以及能證明任意 contract-v1 實作的 <code>make test-conformance</code> 執行器；每一次壓縮拒絕都會送達匯流排（<code>compact:unavailable|failed|declined|invalid|stale</code>，各帶可讀的 <code>detail</code>），先裁剪後壓縮的對話也能再次壓縮——所提供的裁剪點集合與提交守衛已對齊",
      "status.l82": "subagent 續聊 + fork——<code>agent_run</code>/<code>agent_spawn {session}</code> 讓既有子對話再跑一回合（僅附加歷史、基於持久血緣關係的授權、busy 與排隊兩種語義、激活帳本、<code>close: true</code> 退役）；<code>fork</code> 用呼叫方的已完成回合播種全新子對話——從 0 起連續、均衡的裁剪點，失敗即拒絕，出生時快取為冷；未明確指定 <code>model</code> 的新子對話繼承父對話的生效模型及其 provider 釘選",
      ...
      "status.l95": "排序倉庫地圖（repomap 組件）——aider 風格的工作區地圖，由 tree-sitter + 原生 Nim 的 tags 圖經個人化 PageRank 排序得出；<code>repo_map</code> 是 onDemand 且唯讀（模型主動詢問，不受門檻限制），工作區開啟時的自動附加預設開啟但設有準入門檻——低於普查下限（<code>NIF_REPOMAP_MIN_CENSUS</code>，預設 50 個涵蓋檔案）的工作區不建圖，骨架地圖（位元組/符號/檔案門檻）不發布，被攔截的地圖會留下日誌，<code>NIF_REPOMAP_AUTOAPPEND=0</code> 可關閉（<code>bench/reports/repomap-gates-full30.md</code> 驗證門檻會攔截微型倉庫與骨架圖，<code>bench/reports/repomap-ab-*.md</code> 是隨場景而異的 A/B 歷史）",
      ...
      "status.l100": "並行回合分發 + 每 session 事件主題——core 絕不阻塞在 runner 上：每個 session 呼叫（含開啟回合的呼叫）都走自己的私有轉發收件匣，不同對話可以並行推進，而每個 runner 只序列化自己的對話（回合中的呼叫立即以 <code>busy</code> 拒絕）；runner 改發每 session 主題 <code>ev.session.&lt;id&gt;.&lt;kind&gt;</code>——盯住一個對話的客戶端訂閱 <code>ev.session.&lt;id&gt;.&gt;</code>，萬用字元可組合（<code>ev.session.*.token</code>），hooks 按 NATS 語意匹配主題",
      "status.l101": "TS 組件自行解析相依——builder 以 TypeScript 自己的 <code>preProcessFile</code> 掃描入口匯入並 npm 安裝找到的外部套件（跳過相對路徑與 <code>node:</code> 內建模組，≤32 個，名稱先經白名單），解析出的版本範圍寫入產生的 <code>package.json</code>，並在回覆中以 <code>deps</code> 列出；plugins manifest 接受 <code>lang: \"ts\"</code>，TS 外掛套件從此可經 <code>plugin_install</code> 端到端安裝",
      "status.l102": "核准等待保持回應——回合停在核准提示時，等待迴圈會泵送與工具分派相同的閒置表面：等待期間抵達的 session 呼叫立即得到 <code>busy</code> 拒絕，而不是沉默直到 5 分鐘逾時；steer 保持可用；停止指令（<code>__cancel</code>）以拒絕方式中斷等待，回合以取消結束而不是等滿核准逾時；巢狀的核准問題會被立即拒絕——人類一次只回答一個彈窗",
      "status.l103": "雙通道取消讓停止真正落地——停止回合也會中止並行的工具呼叫波（剩餘的每個呼叫各自收到一次 <code>cancel.&lt;component&gt;</code> 發布，並保留部分回覆的常規寬限，與單次分派一致），LLM 重試退避改為約 250 ms 切片泵送，停止在四分之一秒內生效而不是排在整段延遲之後",
      "status.l104": "手動 <code>/compact</code> 的脈絡量表——提交會自行發布一條狀態訊框（reason <code>reset:compact</code>、generation、beforeTokens/afterTokens、usedTokens = 標記 <code>estimated: true</code> 的本地估算），量表立即反映壓縮結果；下一次請求的實測用量會取代該估算，被拒絕的壓縮不發布任何訊框，UI 會提示所示數值為估算值（三種語言均已本地化）",
      "status.l105": "清理失效的互動用戶端——系統目錄每 30 秒依 pid 存活檢查用戶端並剔除死條目（<code>lost (client process gone)</code>——硬退出的 TUI、關閉的終端機、<code>/restart</code> 的後繼者），讓 <code>clientCount</code> 與自動啟動 core 的閒置關機保持真實；pid 重複使用可能短暫保留陳舊條目，ui 登錄檔的租約仍是對話所有權的權威",
      "status.l106": "桌面 UI 變為外掛——<code>ui/</code> 原始碼已遷至 <code>gokr/niffler-ui</code> 倉庫；<code>make install-ui</code> 會啟動一個隔離的自動核准 harness，並經外掛管理器安裝 UI（依最新 release 標籤複製、針對本 harness 建置、發布為 <code>var/bin/niffler-ui</code>），<code>make build</code> 只建置 core + 組件，前端測試也遷入 UI 自己的倉庫——<code>make test</code> 重新成為純粹的匯流排契約套件",
      "status.l107": "manifest v2 套件建置——套件可宣告 <code>{manifestVersion: 2, components: [{name, lang, project, build: {steps, artifact}}]}</code> 並自帶相依清單與鎖檔（<code>package.json</code>/鎖檔、<code>go.mod</code>/<code>go.sum</code>、Nimble）；builder 的 <code>build_package</code> 暫存專案、只展開受控的 SDK/輸出佔位符、執行有界 argv 配方（工具鏈可以組合——例如 <code>npm ci</code> 後接 <code>wails build</code>——但不允許 shell 包裝），並在發布前驗證宣告的產物",
      "status.l108": "小組件的自願自檢——<code>grep</code>、<code>logfile</code>、<code>observe</code> 與 <code>hooks</code> 註冊了 WIRE.md 描述的隱藏 <code>selftest</code>：快速接線檢查，外加 deep 探針——真實的有界 rg 搜尋、寫入真實日誌匯、演練 hooks 去重環、經 stdin 管道送入暫存檔",
      "status.l109": "<code>core.spawn</code> 等到註冊完成才回答——它會等待（<code>NIF_SPAWN_WAIT_MS</code>，預設 5000 毫秒，鉗制在 250–120000），並在被拒絕（工具名衝突——命名空間扁平且全域唯一）或逾時時讓呼叫失敗，錯誤攜帶 catalog 的拒絕原因或逾時說明外加 <code>var/logs/&lt;name&gt;.log</code> 的有界尾段；該次嘗試會被回滾（停止副本、刪除 catalog 條目、不落庫），名稱立即可以重試，<code>cli install</code> 也會點明 plugin_install 回傳的原因",
      "status.l110": "provider 與模型一併釘選——只帶 <code>model</code> 的 <code>session</code> 呼叫會解析該模型所屬的 provider 並在一次標頭寫入中同時持久化兩者，遺留的「半釘選」在 runner 首次載入時自動修復，新子對話在繼承模型的同時也繼承 provider；<code>llm_resolve</code> 失敗時 TUI 標頭會標記過期並重試，而不是把上次的已知狀態當作生效狀態顯示",
      "status.l111": "edit 對登錄檔已知的語言絕不靜默——每次成功編輯後非同步排隊一次 lsp 診斷檢查，編輯結果會點明該通道，或說明檢查為何沒有執行（檔案在對話工作區之外、lsp 不可達）；只有副檔名不被登錄檔認領的檔案才保持沉默",
      "status.l112": "基準 dsh 通道——DeepSeek Harness 加入 harness 基準（可選 <code>--harness dsh</code>，走其 sdk-minimal JSON-RPC 檔）；直連 DeepSeek（無閘道、第一方 API）的全新 full30 配對在 thinking=low 雙雙 30/30，niffler 平均回合更少（6.0 對 6.8）、約 17 秒對 45 秒、39k 對 51k token（<code>bench/reports/full30-direct-dsh-rc3-low-report.md</code>）",
      "status.l113": "store 的 <code>search</code>——第五個 store 工具，各引擎契約一致：<code>search {kind, query, limit?, after?}</code> 回傳與 <code>list</code> 完全相同的形狀與排序（id 升序、同樣的排他游標、limit 預設 100 / 上限 1000），工作階段瀏覽器因此在資料所在處過濾，無需把整個 <code>conversation</code> 類型下載後再本地比對；索引欄位依類型記錄（<code>conversation</code> = id + 標題，<code>message</code> = id + <code>content</code> 下的各字串（上限 16KB），其他類型 = 僅 id），查詢與文字依字母/數字連續段分詞，每個查詢詞都必須是某個已索引詞的大小寫不敏感前綴（AND），因此沒有任何需要轉義的內容；<code>store-sqlite</code>（預設）由 FTS5 索引作答（<code>docs_fts</code>，rowid 與 <code>docs</code> 共用，與文件在同一交易內維護，啟動時若兩者不一致即由 <code>docs</code> 重建——衍生狀態，丟棄無損），<code>barrel</code> 與 <code>store-tidb</code> 則依 id 順序以同一比對器掃描該類型",
      "status.l114": "回合中的影像（多模態）——session 呼叫接受 <code>attachments</code>（<code>{type: \"image\", name?, mimeType, data(base64), width?, height?}</code>）：core 校驗每一項宣告（base64 可解碼、魔數與 PNG/JPEG/GIF/WebP/BMP 允許清單一致、每圖 ≤4 MB、每回合 ≤8 張、每回合 ≤4.5 MB），沒有文字說明的拖入同樣會執行一個回合；請求落地為 OpenAI 多模態 content 陣列、Anthropic 的 base64 <code>source</code> 區塊與 Codex 的 <code>input_image</code> 項——三種協定都能用圖，輸出計費依像素面積而非 base64 長度計價；像素與訊息分開儲存——每張圖一個不含像素的 <code>attachment</code> 中介資料文件加一個 <code>attachmentdata</code> 文件，因為把截圖內嵌進 <code>list</code> 頁會超出匯流排承載上限——訊息只保留參照，投影在 <code>NIF_ATTACH_BUDGET</code> 內保留最新的影像（更早的降級為誠實的文字標記），規則是參照的純函式，因此重啟會重建完全相同的請求，<code>conversation_delete</code> 會隨對話一併清掃像素",
      "status.l115": "匯流排中斷後的重新接入——比 NATS 客戶端自身重連預算更久的中斷不會再讓組件「活著卻失聰」：兩個 SDK 都會探測連線健康（每 2 秒一次 Flush），超過 <code>NIF_RECONNECT_GRACE_S</code>（預設 180 秒，刻意高於客戶端預算）仍不健康時，重新解析匯流排（<code>NIF_NATS_URL</code> → <code>var/nats-url</code> → 眾所周知的連接埠，因此重啟後換了連接埠的新 core 也能找到）、帶退避地重撥、重建所有訂閱並重新發布 <code>reg.publish</code>——無需重啟程序；延遲宣告的組件（<code>mcp-bridge</code>）透過 <code>onReattached</code> 掛鉤重新宣告",
      "status.l116": "上下文計量會預警而不是漂移——被固定的 provider/model 組合若只解析出回退視窗（目錄中沒有該模型），狀態訊框會帶上 <code>warning</code>，點名該組合與實際取得的視窗；<code>warn:threshold</code> 上下文事件帶上 <code>trimAt</code>（以 token 計的有效檔位），客戶端因此能使用 core 列印的同一百分比；終端客戶端會渲染每一種上下文原因——包括壓縮拒絕——而不只是裁剪計數",
      ...
      "status.l57": "<code>make install</code> / <code>make uninstall</code>——將 niffler、niffler-cli、niffler-console 放入 PATH（UI 外掛建置出 <code>var/bin/niffler-ui</code> 後會一併連結；視需求加裝 niffler-tui 包裝器，<code>WITH_TUI=1</code>）；只安裝 niffler 前綴的命令，絕不安裝組件二進位，因此 PATH 不會遮蔽 Unix 工具",
      ...
      "status.t3": "store 的向量記憶——嵌入與相似度召回（全文檢索已落地：SQLite 上的 FTS5 索引，各引擎以掃描提供同一比對器）",
