...
(function () {
  ...
  var CATALOGS = {
    zh: {
      ...
      "comp.th.language": "语言",
      "comp.th.purpose": "用途",
      "comp.core.p": "对话循环、进程管理（supervisor）、组件目录（catalog）和调用分发（dispatch）——以及每次 provider 请求前的上下文窗口准入守卫；<code>session {export: true}</code> 导出下一回合将要发送的确切请求（消息、经允许清单过滤的工具 schema、模型、provider、<code>reasoning_effort</code>），不产生用户消息、LLM 调用或 store 写入；超时或被取消的工具调用会带着部分输出以显式错误返回，而不是被记成静默的成功",
    ...
    "zh-TW": {
      ...
      "comp.th.language": "語言",
      "comp.th.purpose": "用途",
      "comp.core.p": "對話迴圈、行程管理（supervisor）、組件目錄（catalog）與呼叫分派（dispatch）——以及每次 provider 請求前的上下文視窗准入守衛；<code>session {export: true}</code> 匯出下一回合將要發送的確切請求（訊息、經允許清單過濾的工具 schema、模型、provider、<code>reasoning_effort</code>），不產生使用者訊息、LLM 呼叫或 store 寫入；逾時或被取消的工具呼叫會帶著部分輸出以明確錯誤回傳，而不是被記成靜默的成功",
