...
(function () {
  ...
  var CATALOGS = {
    zh: {
      ...
      "why.3p": "agent 写源码 → 调用 <code>builder.build</code> → 调用 <code>core.spawn</code> → 新工具立即可用。新增能力本身就是一次工具调用，LLM 可以在对话过程中自行完成整套流程。社区组件也以相同方式安装：<code>plugin_install</code> 会克隆源码、编译并启动；执行前需人工批准，并始终从发布的源码构建。<code>fabric</code> 更进一步：LLM 可以在内嵌的 Nim VM 中编写程序，控制整个回合内的工具调用流程。外部 MCP 服务器也接入同一条总线：<code>mcp</code> 管理器为每个服务器启动一个桥接进程，其工具以普通目录工具的形式出现。",
      "why.4h": "<span class=\"num\">04</span> 能力形态持久化",
      "why.4p": "能力可以跨重启保留：已启动的组件记录在 store 中，并在下次启动时自动恢复。仓库保存可复现的源码；<code>var/bin/</code> 可以重新构建，持久化状态保存在 store 引擎中（默认 <code>var/barrel-db</code>，可通过 <code>NIF_STORE_BACKEND</code> 换用 SQLite 或 TiDB）。",
      "arch.title": "架构",
      ...
      "qs.test": "# 总线契约测试——每个组件一项契约测试 + 冒烟测试 + Go 测试",
      "qs.see": "└─ make run / recover / down / ram / uninstall / dev · 见 docs/MANUAL.md",
      ...
      "comp.store.p": "总线文档存储，使用版本号（rev）进行乐观并发控制——同一契约下引擎可互换：barrel（默认）、SQLite 与 TiDB/MySQL，通过 NIF_STORE_BACKEND 选择",
      ...
      "comp.edit.p": "文件工具：可分页 <code>read</code>、批量 <code>read_many</code>、带守卫回退级联的精确匹配 <code>edit</code>、原子 <code>write</code>、单层撤销；未变更的整文件重读返回精简的 <code>[unchanged]</code> 标记而不重复输出字节（<code>force</code> 或窗口读取可重新输出），文件在对话上次见过后被改动时 <code>edit</code> 以 <code>E_STALE</code> 拒绝，已读状态按（会话，文件）记录；修改操作需人工批准",
      ...
      "comp.fabric.p": "可编程工具调用——LLM 编写 Nim 程序掌控回合内控制流；带目录钉选的类型化包装、命名程序库、宿主并发的 <code>batch</code> 调用、审批清单，以及按 runId 关联的 <code>ev.fabric.*</code> 生命周期事件——组件还自带经实测运行的示例程序与随附的参考文档",
      ...
      "comp.nats.p": "总线服务器本身，从源码构建——components/nats 将官方 nats-server 编译为 <code>var/bin/nats-server</code>；core 优先使用该二进制而非 PATH 安装，因此仅 <code>make build</code> 即可满足总线依赖；内置构建以 8 MiB 载荷上限运行（上游默认 1 MiB），携带完整对话的 LLM 请求也放得下",
      ...
      "screens.tui.desc": "终端聊天客户端（一个插件——安装好的 niffler-tui 包装器可按需启动 harness）",
      ...
      "bench.1p": "10 个 sympy 实例、单轮提交，由官方 <code>swebench</code> 4.1.0 Docker 环境评分——niffler、pi、opencode 在两个模型上同场对比。Niffler 解题数量较少，但每个任务少用 2.3–2.9 倍 token。完整报告：<code>bench/reports/swe-sympy10-pilot-report.md</code>。",
      "bench.2h": "<span class=\"num\">02</span> harness 基准",
      "bench.2p": "harness 对比基准框架：30 道基础版本即失败的题目（17 道核心题、10 道面向 2–10 分钟/单元的中级题，以及 3 道扇出题——机械工作逐项各异或中间数据超大，正是 guest 程序与脚本化批量作业发挥作用的场景），按类型标注（general / fabric / expert / selfextend），全部经参考实现验证通过；逐轮反馈循环、受保护文件防篡改、按 provider 统计 token——niffler / pi / opencode / codewhale × 两个模型、<code>--thinking low|max</code> 矩阵，另有配对的 niffler-expert 变体用于度量咨询同伴。SWE-bench Verified 与 Datacurve 的 DeepSWE（113 道长程任务）作为额外基准接入；<code>bench/container</code> 提供 Docker 任务镜像，<code>bench/launch.mjs</code> 是引导式启动器。完整 17 题矩阵全通道通过（340/340）；中级题集校准为 20/20（niffler 对 pi，GLM low）；在 syn-large 模型（Synthetic 的 GLM-5.3-Flash）上全套题目全部通过——thinking=low 时 niffler 与 pi 两条通道均为 27/27（<code>bench/reports/full27-syn-large-low-report.md</code>）；瘦身后的 trimmed-prefix 运行保持 27/27，high effort 下未命中缓存的输入减少约 40%（<code>bench/reports/full27-syn-large-{low,high}-trimmed-report.md</code>）。",
      "status.title": "状态",
      ...
      "status.l63": "提示词二次瘦身——工具描述只序列化一次（<code>function.description</code>），<code>files</code> 与 <code>grep</code> 一同转为按需加载：冻结提示词前缀从 2479 降至约 1540 token（−38%）",
      "status.l64": "文件工具已读状态——<code>read</code>/<code>read_many</code>/<code>write</code>/<code>edit</code> 按（会话，文件）记录对话上次观察到的字节摘要：未变更的整文件重读返回 <code>[unchanged]</code>，字节在之后被改动时 <code>edit</code> 以 <code>E_STALE</code> 拒绝",
      ...
      "status.l52": "store 引擎——barrel（默认）、SQLite 与 TiDB/MySQL 由 NIF_STORE_BACKEND 选择；同一总线契约、相同的工具",
    ...
    "zh-TW": {
      ...
      "why.3p": "agent 寫原始碼 → 呼叫 <code>builder.build</code> → 呼叫 <code>core.spawn</code> → 新工具立即可用。新增能力本身就是一次工具呼叫，LLM 可以在對話過程中自行完成整套流程。社群組件也以相同方式安裝：<code>plugin_install</code> 會複製原始碼、編譯並啟動；執行前需經人工確認，且一律從發布的原始碼建置。<code>fabric</code> 更進一步：LLM 可以在內嵌的 Nim VM 中撰寫程式，控制整個回合內的工具呼叫流程。外部 MCP 伺服器也接入同一條匯流排：<code>mcp</code> 管理器為每個伺服器啟動一個橋接行程，其工具以普通目錄工具的形式出現。",
      "why.4h": "<span class=\"num\">04</span> 能力形態持久化",
      "why.4p": "能力可以跨重啟保留：已啟動的組件記錄在 store 中，並在下次啟動時自動復原。倉庫保存可重現的原始碼；<code>var/bin/</code> 可以重新建置，持久化狀態保存在 store 引擎中（預設 <code>var/barrel-db</code>，可透過 <code>NIF_STORE_BACKEND</code> 改用 SQLite 或 TiDB）。",
      "arch.title": "架構",
      ...
      "qs.test": "# 匯流排契約測試——每個組件一項契約測試 + 冒煙測試 + Go 測試",
      "qs.see": "└─ make run / recover / down / ram / uninstall / dev · 見 docs/MANUAL.md",
      ...
      "comp.store.p": "匯流排文件儲存，使用版本號（rev）進行樂觀並行控制——同一契約下引擎可互換：barrel（預設）、SQLite 與 TiDB/MySQL，透過 NIF_STORE_BACKEND 選擇",
      ...
      "comp.edit.p": "檔案工具：可分頁 <code>read</code>、批次 <code>read_many</code>、帶守衛回退級聯的精確匹配 <code>edit</code>、原子 <code>write</code>、單層復原；未變更的整檔重讀回傳精簡的 <code>[unchanged]</code> 標記而不重複輸出位元組（<code>force</code> 或視窗讀取可重新輸出），檔案在對話上次見過後被改動時 <code>edit</code> 以 <code>E_STALE</code> 拒絕，已讀狀態按（會話，檔案）記錄；修改操作需經人工確認",
      ...
      "comp.fabric.p": "可程式化工具呼叫——LLM 撰寫 Nim 程式掌控回合內控制流程；帶目錄釘選的型別化包裝、命名程式庫、宿主並行的 <code>batch</code> 呼叫、核准清單，以及依 runId 關聯的 <code>ev.fabric.*</code> 生命週期事件——組件還自帶經實際執行驗證的範例程式與隨附的參考文件",
      ...
      "comp.nats.p": "匯流排伺服器本身，從原始碼建置——components/nats 將官方 nats-server 編譯為 <code>var/bin/nats-server</code>；core 優先使用該執行檔而非 PATH 安裝，因此僅 <code>make build</code> 即可滿足匯流排依賴；內建建置以 8 MiB 承載上限執行（上游預設 1 MiB），攜帶完整對話的 LLM 請求也放得下",
      ...
      "screens.tui.name": "niffler-tui",
      "screens.tui.desc": "終端機聊天客戶端（一個外掛——安裝好的 niffler-tui 包裝器可視需求啟動 harness）",
      ...
      "bench.1p": "10 個 sympy 實例、單輪提交，由官方 <code>swebench</code> 4.1.0 Docker 環境評分——niffler、pi、opencode 在兩個模型上同場對比。Niffler 解題數量較少，但每個任務少用 2.3–2.9 倍 token。完整報告：<code>bench/reports/swe-sympy10-pilot-report.md</code>。",
      "bench.2h": "<span class=\"num\">02</span> harness 基準",
      "bench.2p": "harness 對比基準框架：30 道基礎版本即失敗的題目（17 道核心題、10 道面向 2–10 分鐘/單元的中級題，以及 3 道扇出題——機械工作逐項各異或中間資料超大，正是 guest 程式與腳本化批次作業發揮作用的場景），依類型標註（general / fabric / expert / selfextend），全部經參考實作驗證通過；逐輪回饋迴圈、受保護檔案防竄改、依 provider 統計 token——niffler / pi / opencode / codewhale × 兩個模型、<code>--thinking low|max</code> 矩陣，另有配對的 niffler-expert 變體用於度量諮詢同伴。SWE-bench Verified 與 Datacurve 的 DeepSWE（113 道長程任務）作為額外基準接入；<code>bench/container</code> 提供 Docker 任務映像，<code>bench/launch.mjs</code> 是引導式啟動器。完整 17 題矩陣全通道通過（340/340）；中級題集校準為 20/20（niffler 對 pi，GLM low）；在 syn-large 模型（Synthetic 的 GLM-5.3-Flash）上全套題目全部通過——thinking=low 時 niffler 與 pi 兩條通道均為 27/27（<code>bench/reports/full27-syn-large-low-report.md</code>）；瘦身後的 trimmed-prefix 執行保持 27/27，high effort 下未命中快取的輸入減少約 40%（<code>bench/reports/full27-syn-large-{low,high}-trimmed-report.md</code>）。",
      "status.title": "狀態",
      ...
      "status.l63": "提示詞二次瘦身——工具描述只序列化一次（<code>function.description</code>），<code>files</code> 與 <code>grep</code> 一同改為按需載入：凍結提示詞前綴從 2479 降至約 1540 token（−38%）",
      "status.l64": "檔案工具已讀狀態——<code>read</code>/<code>read_many</code>/<code>write</code>/<code>edit</code> 按（會話，檔案）記錄對話上次觀察到的位元組摘要：未變更的整檔重讀回傳 <code>[unchanged]</code>，位元組在之後被改動時 <code>edit</code> 以 <code>E_STALE</code> 拒絕",
      ...
      "status.l52": "store 引擎——barrel（預設）、SQLite 與 TiDB/MySQL 由 NIF_STORE_BACKEND 選擇；同一匯流排契約、相同的工具",
