...
(function () {
  ...
  var CATALOGS = {
    zh: {
      ...
      "qs.see": "└─ make run / recover / down / uninstall / dev · 见 docs/MANUAL.md",
      ...
      "comp.edit.p": "文件工具：可分页 <code>read</code>、批量 <code>read_many</code>、带守卫回退级联的精确匹配 <code>edit</code>、原子 <code>write</code>、单层撤销；修改操作需人工批准",
      ...
      "comp.mcp.p": "把外部 MCP 服务器（Model Context Protocol）变成总线组件——基于 store 的注册表，带需审批的 <code>mcp_add</code>/<code>mcp_edit</code>/<code>mcp_remove</code>（每次新增/编辑都通过一次真实连接验证）、检索官方 MCP Registry 的 <code>mcp_search</code>，以及在连接时解析 <code>${ENV}</code> 秘密引用（令牌不落库）；服务器的工具成为普通目录工具，经 <code>discover</code> + <code>invoke</code> 调用",
      ...
      "bench.2p": "harness 对比基准框架：27 道基础版本即失败的题目（17 道核心题 + 10 道面向 2–10 分钟/单元的中级题），按类型标注（general / fabric / expert / selfextend），全部经参考实现验证通过；逐轮反馈循环、受保护文件防篡改、按 provider 统计 token——niffler / pi / opencode / codewhale × 两个模型、<code>--thinking low|max</code> 矩阵，另有配对的 niffler-expert 变体用于度量咨询同伴。SWE-bench Verified 与 Datacurve 的 DeepSWE（113 道长程任务）作为额外基准接入；<code>bench/container</code> 提供 Docker 任务镜像，<code>bench/launch.mjs</code> 是引导式启动器。完整 17 题矩阵全通道通过（340/340）；中级题集校准为 20/20（niffler 对 pi，GLM low）。",
      "status.title": "状态",
      ...
      "status.l60": "外部 MCP 服务器——mcp 管理器 + 每个服务器一个受监督桥接进程：工具、提示词和资源成为目录工具与斜杠命令；惰性沙箱会话、进行中调用取消、契约漂移重启、${ENV} 秘密引用、注册表检索，以及 Web UI 的 MCP 管理面板",
    ...
    "zh-TW": {
      ...
      "qs.see": "└─ make run / recover / down / uninstall / dev · 見 docs/MANUAL.md",
      ...
      "comp.edit.p": "檔案工具：可分頁 <code>read</code>、批次 <code>read_many</code>、帶守衛回退級聯的精確匹配 <code>edit</code>、原子 <code>write</code>、單層復原；修改操作需經人工確認",
      ...
      "comp.mcp.p": "把外部 MCP 伺服器（Model Context Protocol）變成匯流排組件——以 store 為基礎的註冊表，帶需核准的 <code>mcp_add</code>/<code>mcp_edit</code>/<code>mcp_remove</code>（每次新增/編輯都透過一次真實連線驗證）、檢索官方 MCP Registry 的 <code>mcp_search</code>，以及在連線時解析 <code>${ENV}</code> 秘密引用（權杖不入庫）；伺服器的工具成為普通目錄工具，經 <code>discover</code> + <code>invoke</code> 呼叫",
      ...
      "bench.2p": "harness 對比基準框架：27 道基礎版本即失敗的題目（17 道核心題 + 10 道面向 2–10 分鐘/單元的中級題），依類型標註（general / fabric / expert / selfextend），全部經參考實作驗證通過；逐輪回饋迴圈、受保護檔案防竄改、依 provider 統計 token——niffler / pi / opencode / codewhale × 兩個模型、<code>--thinking low|max</code> 矩陣，另有配對的 niffler-expert 變體用於度量諮詢同伴。SWE-bench Verified 與 Datacurve 的 DeepSWE（113 道長程任務）作為額外基準接入；<code>bench/container</code> 提供 Docker 任務映像，<code>bench/launch.mjs</code> 是引導式啟動器。完整 17 題矩陣全通道通過（340/340）；中級題集校準為 20/20（niffler 對 pi，GLM low）。",
      "status.title": "狀態",
      ...
      "status.l60": "外部 MCP 伺服器——mcp 管理器 + 每個伺服器一個受監督橋接行程：工具、提示詞和資源成為目錄工具與斜線命令；惰性沙箱工作階段、進行中呼叫取消、契約漂移重啟、${ENV} 秘密引用、註冊表檢索，以及 Web UI 的 MCP 管理面板",
