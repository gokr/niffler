...
(function () {
  ...
  var CATALOGS = {
    zh: {
      ...
      "status.l45": "提示词缓存经济——每个 session 状态事件携带累计 <code>cache {prompt, read, hitRate}</code> 拆分，UI 每条消息显示 <code>⚡ NN% cached</code>",
    ...
    "zh-TW": {
      ...
      "status.l45": "提示詞快取經濟——每個 session 狀態事件攜帶累計 <code>cache {prompt, read, hitRate}</code> 拆分，UI 每則訊息顯示 <code>⚡ NN% cached</code>",
