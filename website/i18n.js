...
(function () {
  ...
  var CATALOGS = {
    zh: {
      ...
      "why.3p": "agent 写源码 → 调用 <code>builder.build</code> → 调用 <code>core.spawn</code> → 工具上线。添加能力就是一次工具调用，而且 LLM 在对话中途对自己完成它。社区包以同样的方式安装：<code>plugin_install</code> clone、从源码编译并启动——经审批门控，始终从发布的代码构建。<code>fabric</code> 更进一步：LLM 在内嵌 Nim VM 里为整段控制流编程。",
      ...
      "qs.test": "# 总线契约套件——21 个契约测试 + smoke + go 测试",
      ...
      "comp.fabric.p": "可编程工具调用——LLM 编写一段拥有回合内控制流的 Nim 程序，在内嵌 VM 中运行并可调用总线工具（<code>callTool</code>）；只有结果进入对话",
      "comp.agent.p": "子代理会话——把任意对话变成委派的子 runner，拥有全新上下文；摘要返回给调用方",
      ...
      "status.l27": "fabric——可编程工具调用：LLM 编写的 Nim 程序运行在 VM 内嵌执行器中，可从 guest 内部调用总线工具",
      "status.l28": "子代理会话——委派子 runner、全新上下文，摘要返回给调用方",
      "status.l29": "斜杠命令——面向 UI 的 <code>niffler.json</code> 声明式注册表，持久化到 store",
      "status.l30": "思考力度——每会话 <code>reasoning_effort</code>（ctrl+g），TUI 流式推理渲染",
    ...
    "zh-TW": {
      ...
      "why.3p": "agent 寫原始碼 → 呼叫 <code>builder.build</code> → 呼叫 <code>core.spawn</code> → 工具上線。新增能力就是一次工具呼叫，而且 LLM 在對話中途對自己完成它。社群套件以同樣的方式安裝：<code>plugin_install</code> clone、從原始碼編譯並啟動——經核准門控，始終從發布的程式碼建置。<code>fabric</code> 更進一步：LLM 在內嵌 Nim VM 裡為整段控制流程編程。",
      ...
      "qs.test": "# 匯流排契約套件——21 個契約測試 + smoke + go 測試",
      ...
      "comp.fabric.p": "可程式化工具呼叫——LLM 撰寫一段擁有回合內控制流程的 Nim 程式，在內嵌 VM 中執行並可呼叫匯流排工具（<code>callTool</code>）；只有結果進入對話",
      "comp.agent.p": "子代理對話——把任意對話變成委派的子 runner，擁有全新上下文；摘要回傳給呼叫方",
      ...
      "status.l27": "fabric——可程式化工具呼叫：LLM 撰寫的 Nim 程式在 VM 內嵌執行器中執行，可從 guest 內部呼叫匯流排工具",
      "status.l28": "子代理對話——委派子 runner、全新上下文，摘要回傳給呼叫方",
      "status.l29": "斜線命令——面向 UI 的 <code>niffler.json</code> 宣告式註冊表，持久化到 store",
      "status.l30": "思考力度——每對話 <code>reasoning_effort</code>（ctrl+g），TUI 串流推理渲染",
