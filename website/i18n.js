...
(function () {
  ...
  var CATALOGS = {
    zh: {
      ...
      "comp.nats.p": "总线服务器本身，从源码构建——components/nats 将官方 nats-server 编译为 <code>var/bin/nats-server</code>；core 优先使用该二进制而非 PATH 安装，因此仅 <code>make build</code> 即可满足总线依赖",
      ...
      "bench.2p": "harness 对比基准框架：17 道基础版本即失败的题目，按类型标注（general / fabric / expert / selfextend），全部经参考实现验证通过；逐轮反馈循环、受保护文件防篡改、按 provider 统计 token——niffler / pi / opencode / codewhale × 两个模型、<code>--thinking low|max</code> 矩阵，另有配对的 niffler-expert 变体用于度量咨询同伴。",
      "status.title": "状态",
    ...
    "zh-TW": {
      ...
      "comp.nats.p": "匯流排伺服器本身，從原始碼建置——components/nats 將官方 nats-server 編譯為 <code>var/bin/nats-server</code>；core 優先使用該執行檔而非 PATH 安裝，因此僅 <code>make build</code> 即可滿足匯流排依賴",
      ...
      "bench.2p": "harness 對比基準框架：17 道基礎版本即失敗的題目，依類型標註（general / fabric / expert / selfextend），全部經參考實作驗證通過；逐輪回饋迴圈、受保護檔案防竄改、依 provider 統計 token——niffler / pi / opencode / codewhale × 兩個模型、<code>--thinking low|max</code> 矩陣，另有配對的 niffler-expert 變體用於度量諮詢同伴。",
      "status.title": "狀態",
