...
(function () {
  ...
  var CATALOGS = {
    zh: {
      ...
      "status.l100": "审批等待保持响应——回合停在审批提示时，等待循环会泵送与工具分发相同的空闲表面：等待期间到达的 session 调用立即得到 <code>busy</code> 拒绝，而不是沉默直到 5 分钟超时；steer 保持可用；停止指令（<code>__cancel</code>）以拒绝方式中断等待，回合以取消结束而不是等满审批超时；嵌套的审批问题会被立即拒绝——人类一次只回答一个弹窗",
      "status.l101": "双通道取消让停止真正落地——停止回合也会中止并行的工具调用波（剩余的每个调用各自收到一次 <code>cancel.&lt;component&gt;</code> 发布，并保留部分回复的常规宽限，与单次分发一致），LLM 重试退避改为约 250 ms 切片泵送，停止在四分之一秒内生效而不是排在整段延迟之后",
      "status.l102": "手动 <code>/compact</code> 的上下文量表——提交会自行发布一条状态帧（reason <code>reset:compact</code>、generation、beforeTokens/afterTokens、usedTokens = 标记 <code>estimated: true</code> 的本地估算），量表立即反映压缩结果；下一次请求的实测用量会替换该估算，被拒绝的压缩不发布任何帧，UI 会提示所示数值为估算值（三种语言均已本地化）",
      "status.l103": "清理失效的交互客户端——系统目录每 30 秒按 pid 存活检查客户端并剔除死条目（<code>lost (client process gone)</code>——硬退出的 TUI、关闭的终端、<code>/restart</code> 的后继者），让 <code>clientCount</code> 与自动启动 core 的空闲关停保持真实；pid 复用可能短暂保留陈旧条目，ui 注册表的租约仍是对话所有权的权威",
    ...
    "zh-TW": {
      ...
      "status.l100": "核准等待保持回應——回合停在核准提示時，等待迴圈會泵送與工具分派相同的閒置表面：等待期間抵達的 session 呼叫立即得到 <code>busy</code> 拒絕，而不是沉默直到 5 分鐘逾時；steer 保持可用；停止指令（<code>__cancel</code>）以拒絕方式中斷等待，回合以取消結束而不是等滿核准逾時；巢狀的核准問題會被立即拒絕——人類一次只回答一個彈窗",
      "status.l101": "雙通道取消讓停止真正落地——停止回合也會中止並行的工具呼叫波（剩餘的每個呼叫各自收到一次 <code>cancel.&lt;component&gt;</code> 發布，並保留部分回覆的常規寬限，與單次分派一致），LLM 重試退避改為約 250 ms 切片泵送，停止在四分之一秒內生效而不是排在整段延遲之後",
      "status.l102": "手動 <code>/compact</code> 的脈絡量表——提交會自行發布一條狀態訊框（reason <code>reset:compact</code>、generation、beforeTokens/afterTokens、usedTokens = 標記 <code>estimated: true</code> 的本地估算），量表立即反映壓縮結果；下一次請求的實測用量會取代該估算，被拒絕的壓縮不發布任何訊框，UI 會提示所示數值為估算值（三種語言均已本地化）",
      "status.l103": "清理失效的互動用戶端——系統目錄每 30 秒依 pid 存活檢查用戶端並剔除死條目（<code>lost (client process gone)</code>——硬退出的 TUI、關閉的終端機、<code>/restart</code> 的後繼者），讓 <code>clientCount</code> 與自動啟動 core 的閒置關機保持真實；pid 重複使用可能短暫保留陳舊條目，ui 登錄檔的租約仍是對話所有權的權威",
