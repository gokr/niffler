...
(function () {
  ...
  var CATALOGS = {
    zh: {
      ...
      "bench.2p": "harness 对比基准框架：30 道基础版本即失败的题目（17 道核心题、10 道面向 2–10 分钟/单元的中级题，以及 3 道扇出题——机械工作逐项各异或中间数据超大，正是 guest 程序与脚本化批量作业发挥作用的场景），按类型标注（general / fabric / expert / selfextend），全部经参考实现验证通过；逐轮反馈循环、受保护文件防篡改、按 provider 统计 token——niffler / pi / opencode / codewhale × 两个模型、<code>--thinking low|max</code> 矩阵，另有配对的 niffler-expert 变体用于度量咨询同伴。SWE-bench Verified 与 Datacurve 的 DeepSWE（113 道长程任务）作为额外基准接入；<code>bench/container</code> 提供 Docker 任务镜像，<code>bench/launch.mjs</code> 是引导式启动器。完整 17 题矩阵全通道通过（340/340）；中级题集校准为 20/20（niffler 对 pi，GLM low）；在完整的 30 题题集上，syn-large（Synthetic 的 GLM-5.3-Flash）与 deepseek-v4.1-flash 全部通过——两个模型在 thinking=low 时 niffler 均为 30/30，pi 在 deepseek 的 low 与 high 档均为 30/30（<code>bench/reports/full30-{syn-large,deepseek-v4.1-flash}-*-report.md</code>）；瘦身后的 trimmed-prefix 运行保持全绿，high effort 下未命中缓存的输入减少约 40%（<code>bench/reports/full27-syn-large-{low,high}-trimmed-report.md</code>）。",
      "status.title": "状态",
    ...
    "zh-TW": {
      ...
      "bench.2p": "harness 對比基準框架：30 道基礎版本即失敗的題目（17 道核心題、10 道面向 2–10 分鐘/單元的中級題，以及 3 道扇出題——機械工作逐項各異或中間資料超大，正是 guest 程式與腳本化批次作業發揮作用的場景），依類型標註（general / fabric / expert / selfextend），全部經參考實作驗證通過；逐輪回饋迴圈、受保護檔案防竄改、依 provider 統計 token——niffler / pi / opencode / codewhale × 兩個模型、<code>--thinking low|max</code> 矩陣，另有配對的 niffler-expert 變體用於度量諮詢同伴。SWE-bench Verified 與 Datacurve 的 DeepSWE（113 道長程任務）作為額外基準接入；<code>bench/container</code> 提供 Docker 任務映像，<code>bench/launch.mjs</code> 是引導式啟動器。完整 17 題矩陣全通道通過（340/340）；中級題集校準為 20/20（niffler 對 pi，GLM low）；在完整的 30 題題集上，syn-large（Synthetic 的 GLM-5.3-Flash）與 deepseek-v4.1-flash 全部通過——兩個模型在 thinking=low 時 niffler 均為 30/30，pi 在 deepseek 的 low 與 high 檔均為 30/30（<code>bench/reports/full30-{syn-large,deepseek-v4.1-flash}-*-report.md</code>）；瘦身後的 trimmed-prefix 執行保持全綠，high effort 下未命中快取的輸入減少約 40%（<code>bench/reports/full27-syn-large-{low,high}-trimmed-report.md</code>）。",
      "status.title": "狀態",
