...
(function () {
  ...
  var CATALOGS = {
    zh: {
      ...
      "qs.test": "# 总线契约测试——21 项契约测试 + 冒烟测试 + Go 测试",
      ...
      "comp.git.p": "只读仓库检查：<code>status</code>/<code>diff</code>/<code>log</code>/<code>show</code>/<code>blame</code>；无需审批，固定 argv，路径限定在 harness 根目录",
      ...
      "comp.fabric.p": "可编程工具调用——LLM 编写一段控制回合内工具调用流程的 Nim 程序，由内嵌 VM 执行并通过 <code>callTool</code> 调用总线工具；只有最终结果进入对话上下文",
      "comp.agent.p": "subagent session——将任务委派给拥有全新上下文的子 runner，并把结果摘要返回给调用方",
    ...
    "zh-TW": {
      ...
      "qs.test": "# 匯流排契約測試——21 項契約測試 + 冒煙測試 + Go 測試",
      ...
      "comp.git.p": "唯讀倉庫檢查：<code>status</code>/<code>diff</code>/<code>log</code>/<code>show</code>/<code>blame</code>；無需核准，固定 argv，路徑限定在 harness 根目錄",
      ...
      "comp.fabric.p": "可程式化工具呼叫——LLM 撰寫一段控制回合內工具呼叫流程的 Nim 程式，由內嵌 VM 執行並透過 <code>callTool</code> 呼叫匯流排工具；只有最終結果進入對話脈絡",
      "comp.agent.p": "subagent session——將任務委派給擁有全新脈絡的子 runner，並把結果摘要回傳給呼叫方",
