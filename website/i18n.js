...
(function () {
  ...
  var CATALOGS = {
    zh: {
      ...
      "comp.lsp.p": "语言服务器接缝——一个 <code>lsp</code> 工具（无需跑测试即可获得 <code>diagnostics</code>、<code>goToDefinition</code>、<code>findReferences</code>、<code>goToImplementation</code>、<code>hover</code>），可对接任意已配置的 stdio 语言服务器（默认 gopls、nimlangserver、tsserver、pyright、rust-analyzer、clangd、bash-LSP）；注册表是数据（<code>$XDG_CONFIG_HOME/niffler-lsp/servers.json</code>）——新增一门语言只是加一条配置，或由 agent 自己发起需审批的 <code>lsp_registry add</code>，绝不需要写代码；只读的 <code>lsp_servers</code> 列出合并后的注册表，niffler-tui 的 <code>/lsp</code> 可浏览并编辑它",
      ...
      "status.l72": "语言服务器接缝——一个 <code>lsp</code> 工具（diagnostics/定义/引用/实现/hover）可对接任意已配置的 stdio 服务器，注册表即数据（7 个内置服务器）、只读 <code>lsp_servers</code>、需审批的 <code>lsp_registry add</code>，以及 niffler-tui 的 <code>/lsp</code> 选择器",
      ...
      "status.l76": "基准——SWE-bench Multilingual 试点（10 题、7 种语言）在配套的 <code>swebench</code> 5.0.2 虚拟环境中运行，用于内嵌评测规范的 dataset；claudecode 通道加入 harness 基准（full30：30/30 对 30/30，niffler 每回合更精简）；在 DeepSeek V4.1 Flash（Synthetic，thinking=high）上 niffler 以 10/10 对 claudecode 8/10 拿下 Sym10 试点",
      "status.l77": "流式节奏——附着 token 流时 dispatch 以 5 ms 轮询（其余 100 ms），token 增量按生产速度渲染，不再在 reply-wait 边界被批量合并",
    ...
    "zh-TW": {
      ...
      "comp.lsp.p": "語言伺服器接縫——一個 <code>lsp</code> 工具（不必跑測試即可取得 <code>diagnostics</code>、<code>goToDefinition</code>、<code>findReferences</code>、<code>goToImplementation</code>、<code>hover</code>），可對接任意已設定的 stdio 語言伺服器（預設 gopls、nimlangserver、tsserver、pyright、rust-analyzer、clangd、bash-LSP）；登錄檔是資料（<code>$XDG_CONFIG_HOME/niffler-lsp/servers.json</code>）——新增一門語言只是加一筆設定，或由 agent 自己發起需核准的 <code>lsp_registry add</code>，絕不需要寫程式；唯讀的 <code>lsp_servers</code> 列出合併後的登錄檔，niffler-tui 的 <code>/lsp</code> 可瀏覽並編輯它",
      ...
      "status.l72": "語言伺服器接縫——一個 <code>lsp</code> 工具（diagnostics/定義/引用/實作/hover）可對接任意已設定的 stdio 伺服器，登錄檔即資料（7 個內建伺服器）、唯讀 <code>lsp_servers</code>、需核准的 <code>lsp_registry add</code>，以及 niffler-tui 的 <code>/lsp</code> 選擇器",
      ...
      "status.l76": "基準——SWE-bench Multilingual 試點（10 題、7 種語言）在配套的 <code>swebench</code> 5.0.2 虛擬環境中執行，用於內嵌評測規範的 dataset；claudecode 通道加入 harness 基準（full30：30/30 對 30/30，niffler 每回合更精簡）；在 DeepSeek V4.1 Flash（Synthetic，thinking=high）上 niffler 以 10/10 對 claudecode 8/10 拿下 Sym10 試點",
      "status.l77": "串流節奏——附著 token 串流時 dispatch 以 5 ms 輪詢（其餘 100 ms），token 增量按生產速度呈現，不再在 reply-wait 邊界被批次合併",
