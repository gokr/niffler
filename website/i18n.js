...
(function () {
  ...
  var CATALOGS = {
    zh: {
      ...
      "comp.builder.p": "将 agent 编写的 Nim/Go/TypeScript 源码编译为可执行程序——依赖由源码声明，而非构建参数：Go 构建用 <code>go mod tidy</code> 解析导入，TS 构建在基础安装之后扫描入口的导入并 npm 安装外部包（解析出的版本范围写入生成的 <code>package.json</code>，回复中以 <code>deps</code> 列出）",
      ...
      "comp.plugins.p": "组件生态：按 topic 搜索，通过 <code>niffler.json</code> 描述包，支持安装/更新/移除；始终从源码构建；提供按组件名加前缀的 <code>/plugins</code>/<code>/plugins-search</code>/<code>/plugins-install</code>/<code>/plugins-update</code>/<code>/plugins-remove</code> 斜杠命令；仓库没有发布 release 时标签解析会回退到它的 tags，分支钉选的安装会跟随分支原地更新，不会被改指到 release 标签；TypeScript 包也能安装了——manifest 读取器接受 <code>lang: \"ts\"</code>，条目经 builder 构建并像其他组件一样启动",
    ...
    "zh-TW": {
      ...
      "comp.builder.p": "將 agent 撰寫的 Nim/Go/TypeScript 原始碼編譯為執行檔——相依由原始碼宣告，而非建置參數：Go 建置以 <code>go mod tidy</code> 解析匯入，TS 建置在基礎安裝之後掃描入口的匯入並 npm 安裝外部套件（解析出的版本範圍寫入產生的 <code>package.json</code>，回覆中以 <code>deps</code> 列出）",
      ...
      "comp.plugins.p": "組件生態：依 topic 搜尋，透過 <code>niffler.json</code> 描述套件，支援安裝/更新/移除；一律從原始碼建置；提供依組件名加前綴的 <code>/plugins</code>/<code>/plugins-search</code>/<code>/plugins-install</code>/<code>/plugins-update</code>/<code>/plugins-remove</code> 斜線命令；倉庫沒有發布 release 時標籤解析會回退到它的 tags，分支釘選的安裝會沿分支原地更新，不會被改指到 release 標籤；TypeScript 套件也能安裝了——manifest 讀取器接受 <code>lang: \"ts\"</code>，條目經 builder 建置並像其他組件一樣啟動",
