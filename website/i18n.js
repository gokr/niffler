...
(function () {
  ...
  var CATALOGS = {
    zh: {
      ...
      "bench.2p": "harness 对比基准框架：17 道基础版本即失败的题目，按类型标注（general / fabric / expert / selfextend），全部经参考实现验证通过；逐轮反馈循环、受保护文件防篡改、按 provider 统计 token——niffler / pi / opencode / codewhale × 两个模型、<code>--thinking low|max</code> 矩阵，另有配对的 niffler-expert 变体用于度量咨询同伴。SWE-bench Verified 与 Datacurve 的 DeepSWE（113 道长程任务）作为额外基准接入；<code>bench/container</code> 提供 Docker 任务镜像，<code>bench/launch.mjs</code> 是引导式启动器。最新完整矩阵：全部 340 个单元通过。",
      "status.title": "状态",
    ...
    "zh-TW": {
      ...
      "bench.2p": "harness 對比基準框架：17 道基礎版本即失敗的題目，依類型標註（general / fabric / expert / selfextend），全部經參考實作驗證通過；逐輪回饋迴圈、受保護檔案防竄改、依 provider 統計 token——niffler / pi / opencode / codewhale × 兩個模型、<code>--thinking low|max</code> 矩陣，另有配對的 niffler-expert 變體用於度量諮詢同伴。SWE-bench Verified 與 Datacurve 的 DeepSWE（113 道長程任務）作為額外基準接入；<code>bench/container</code> 提供 Docker 任務映像，<code>bench/launch.mjs</code> 是引導式啟動器。最新完整矩陣：全部 340 個單元通過。",
      "status.title": "狀態",
