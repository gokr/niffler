...
Core + 組件需要 **Nim**、**Go** 和 **nats-server**；TypeScript 組件和
...
# nats-server —— 或從 https://github.com/nats-io/nats-server/releases 拿二進位檔
...
本身會重用預設連接埠上已在執行的匯流排，否則在那裡拉起 nats-server
（4222 被佔用時退回隨機回送連接埠）並寫 `var/nats-url`。設定
