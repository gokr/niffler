...
let taskTimeoutMs = Number(opt("task-timeout-min", cfg.defaults.taskTimeoutMin)) * 60_000;
...
async function runTask(combo, taskId, taskMeta, taskPrompt, shared) {
  ...
  const isTransportError = (msg) => {
    ...
    // A session-call/turn timeout is deterministic budget exhaustion, not a
    // flaky wire — the identical wait would just burn the budget again
    // (seen: glm max turns > 10 min → 3×630s per round, then fail).
    // Genuine transport failures carry a connection-level marker.
    if (/round timed out|call session timed out/i.test(m)) return false;
