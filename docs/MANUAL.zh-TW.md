...
> 🤖 AI 自動翻譯，可能滯後於英文版；以 [English](MANUAL.md) 為準。
...
- **`/compact`** —— 立即執行壓縮器，而不是等待自動壓力階梯：core 向已設定的
  ...
  使用者訊息。輔助摘要繼承會話已解析的 provider/model，不會靜默跟隨之後的全域
  provider 切換。回覆報告 `compacted: true` 及前後 token 數，或
...
普通 GitHub 倉庫（一個倉庫 = 一個包 = N 個元件）。帶 GitHub topic
...
| `plugin_install {repo, version?}` | clone 到 `var/plugins/<pkg>@<ref>/`，經 builder 的 `build` 工具從原始碼建置每個元件，然後 `spawn` 每個服務元件（需審批） |
