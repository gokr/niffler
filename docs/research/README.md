...
| [NATSNIM.md](NATSNIM.md) | The pure-Nim NATS client ([gokr/natsnim](https://github.com/gokr/natsnim), a translation of `nats-io/nats.go`): why it replaces the `nats.c` binding, how the `-d:nifflerNimNats` switch aliases the shim so no call site changes, what P7 found (a 1 ms `NextMsg` that never read the socket, and missing no-responders), and what is left before `libnats` can leave the prerequisite list |
