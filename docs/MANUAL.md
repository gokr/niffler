...
| `repomap` | Nim | optional | ranked workspace map (docs/research/REPOMAP.md): the load-bearing files and their key definitions in ~1KB, built from a tree-sitter + native-Nim tags graph with personalized PageRank (the aider repomap port). Appended to every conversation automatically on workspace open (one append-only entry; the component publishes it, the runner appends it) — `repo_map {workspace?, focus?, mentionedIdents?, budget?}` is the onDemand refresh. Cache: `var/repomap-tags/` (mtime-keyed). Optional component — absent means no map, nothing else changes |
