...
| `attachment` | `<messageId>:a<i>` | one dropped image's pixels (base64 `data` plus message id, MIME, dimensions, original name) kept out of the message document so `list` pages stay under the bus payload limit. Written before the message that refs it; read when the context materializes images for the provider request (newest-first within `NIF_ATTACH_BUDGET`), and swept by `conversation_delete` |
