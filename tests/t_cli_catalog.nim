...
proc main() =
  ...
  block:
    ...
    if code == -1: cli.kill()
    let output = cli.outputStream.readAll()
    check("call uses core’s catalog-selected service", code == 0 and output.contains("\"pong\":true"), output)

  for response in ["not-json", "core-error", "{}", "{\"components\":[]}"]:
    let cli = startComponent(cliBin, url, root = root, args = ["catalog"])
    defer: stopProcess(cli)
    let request = nextCall(core)
    announce()
    let data = if response == "not-json": response
               elif response == "core-error": errorEnvelope(request.env.id, "unavailable", "fixture").encode()
  ...
  block:
    ...
    let unavailable = nextCall(core)
    nc.publish(unavailable.reply, errorEnvelope(unavailable.env.id,
      "unavailable", "fixture").encode())
