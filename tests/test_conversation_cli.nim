...
suite "CLI Command Integration Tests":
  ...
  testConversationLifecycle "New conversation command creates and switches":
    ...
    let result2 = executeCommand("new", @["Test", "Conversation", "Title"], testModel)
  ...
  testConversationLifecycle "Conversation switching by ID and title":
    ...
    let switchById = executeCommand("conv", @[$conv2Id], testModel)
    ...
    let switchByTitle = executeCommand("conv", @["First"], testModel)
    ...
    let switchExact = executeCommand("conv", @["Second", "Conversation"], testModel)
    check switchExact.success == true
    check "Switched to conversation: Second Conversation" in switchExact.message
    
    # Test invalid conversation ID
    let switchInvalid = executeCommand("conv", @["999"], testModel)
    ...
    let switchNonExistent = executeCommand("conv", @["NonExistent"], testModel)
  ...
  testConversationLifecycle "Archive command functionality":
    ...
    let archiveResult = executeCommand("archive", @[$conv1Id], testModel)
    ...
    let invalidArchive = executeCommand("archive", @["999"], testModel)
    ...
    let noArgs = executeCommand("archive", @[], testModel)
  ...
  testConversationLifecycle "Search command with database content matching":
    ...
    let pythonResults = executeCommand("search", @["Python"], testModel)
    ...
    let testingResults = executeCommand("search", @["testing"], testModel)
    ...
    let venvResults = executeCommand("search", @["virtual", "environments"], testModel)
    check venvResults.success == true
    check "Python Development Setup" in venvResults.message
    
    # Test search with no results
    let noResults = executeCommand("search", @["nonexistent"], testModel)
    ...
    let noArgs = executeCommand("search", @[], testModel)
    check noArgs.success == false
  ...
  testConversationLifecycle "Info command shows current conversation details":
    ...
    let infoResult = executeCommand("info", @[], testModel)
...
suite "Concurrent Operations and Thread Safety":
  ...
  testConversationLifecycle "Sequential conversation operations maintain consistency":
    ...
    for conv in conversations:
      let switchResult = executeCommand("conv", @[$conv.id], testModel)
