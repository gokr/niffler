...
proc recordTokenCountCorrection*(modelNickname: string, estimatedTokens: int, actualTokens: int) =
  ...
  debug(fmt"🔍 recordTokenCountCorrection ENTRY: model={modelNickname}, estimated={estimatedTokens}, actual={actualTokens}")  
  if estimatedTokens <= 0 or actualTokens <= 0:
    debug(fmt"❌ Invalid parameters for recordTokenCountCorrection: model={modelNickname}, estimated={estimatedTokens}, actual={actualTokens}")
