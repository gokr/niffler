...
proc generatedPluginPath(path: string): bool =
  ## Ignore files generated by normal manual builds when checking whether the
  ## package source was edited locally.
  let p = path.replace('\\', '/')
  p == "go.work" or p == "go.work.sum" or p == "package-lock.json" or
    p == "yarn.lock" or p == "pnpm-lock.yaml" or
    p.startsWith("bin/") or p.startsWith("dist/") or
    p.startsWith("node_modules/") or p.startsWith("target/") or
    p.startsWith("nimcache/") or p.startsWith("build/")

proc repoHasSourceChanges(dir: string): bool =
  ## Detect tracked edits and meaningful untracked files in the clone. The
  ## generated go.work/build outputs are explicitly not source changes.
  let r = runCmd("git -C " & quoteShell(dir) &
                 " status --porcelain --untracked-files=all", 15_000)
  if r.code != 0: return true
  for line in r.output.splitLines():
    if line.len < 3: continue
    var path = line[3 .. ^1].strip()
    let arrow = path.rfind(" -> ")
    if arrow >= 0: path = path[arrow + 4 .. ^1].strip()
    if line[0 .. 1] == "??" and generatedPluginPath(path): continue
    return true
  false

proc artifactsNeedRebuild(rec: JsonNode, commit: string): bool =
  ## Old records have no builtCommit, so they get one repair build. Later
  ## no-op pulls remain cheap while still repairing missing binaries, runtime
  ## bundles, or locally edited package sources.
  if commit.len == 0 or rec{"builtCommit"}.getStr("") != commit:
    return true
  if repoHasSourceChanges(rec{"dir"}.getStr("")): return true
  let comps = rec{"components"}
  if comps == nil or comps.kind != JArray or comps.len == 0: return true
  for item in comps:
    let binary = item{"binary"}.getStr("")
    if binary.len == 0 or not fileExists(binary): return true
    let runtime = item{"runtime"}.getStr("")
    if runtime.len > 0 and not dirExists(runtime): return true
  false
...
proc rebuildPackage(rec: JsonNode, branch, commit: string,
                    sourceUpdated: bool): JsonNode =
  ## Build a package at its current checkout and replace supervised services
  ## only after every component builds. This is also the repair path for an
  ## old install record or a missing interactive binary when git pull is a
  ## no-op.
  let dest = rec{"dir"}.getStr("")
  if dest.len == 0 or not dirExists(dest):
    return errResult("package directory missing: " & dest)
  if commit.len == 0:
    return errResult("cannot record the package checkout commit")
  var mf: Manifest
  try:
    mf = readManifest(dest)
  except CatchableError as e:
    return errResult("manifest invalid; existing components remain running: " & e.msg)
  if mf.manifestVersion == 1: writeGoWork(dest, mf)
  let built = buildComponents(mf, dest, root() / "var" / "bin")
  if not built.ok:
    cleanupBuilt(built.items)
    return errResult(
      (if sourceUpdated: "pulled new commits but build failed; old components remain running"
       else: "installed artifacts are stale and rebuild failed; old components remain running"),
      extra = %*{"components": built.items, "commit": commit})
  if not publishBuilt(mf, built.items):
    cleanupBuilt(built.items)
    return errResult(
      (if sourceUpdated: "pulled new commits but publish failed; old components remain running"
       else: "installed artifacts are stale and publish failed; old components remain running"),
      extra = %*{"components": built.items, "commit": commit})
  ...
  for st in components:
    if st{"spawned"}.getBool(false) or st{"interactive"}.getBool(false):
      inc installed
  if installed == 0:
    return errResult("no component could be rebuilt",
                     extra = %*{"components": components, "removed": removed,
                                "commit": commit})
  saveRecord(mf.name, %*{"name": mf.name,
                         "repo": rec{"repo"}.getStr(""),
                         "ref": branch, "dir": dest, "version": mf.version,
                         "manifestVersion": mf.manifestVersion,
                         "builtCommit": commit, "components": components,
                         "addedAt": rec{"addedAt"}.getFloat(epochTime())})
  result = okResult(%*{"package": mf.name, "repo": rec{"repo"}.getStr(""),
                       "ref": branch, "commit": commit,
                       "updated": sourceUpdated, "rebuilt": true,
                       "repaired": not sourceUpdated,
                       "components": components, "removed": removed})
...
proc doUpdateBranch(pkg: string, rec: JsonNode): JsonNode =
  ...
  ## tag. A no-op pull stays cheap only when the checkout and its recorded
  ## artifacts are current; old records and missing/dirty artifacts take the
  ## repair build path.
  let dest = rec{"dir"}.getStr("")
  ...
  let before = repoHead(dest)
  if before.len == 0:
    return errResult("git rev-parse failed")
  ...
  let after = repoHead(dest)
  if after.len == 0:
    return errResult("git rev-parse failed after pull")
  let needsRepair = artifactsNeedRebuild(rec, after)
  if after == before and not needsRepair:
    if branch != rec{"ref"}.getStr(""):
      ...
      try:
        ...
        discard
    return okResult(%*{"updated": false, "rebuilt": false,
                       "ref": branch, "commit": after})
  return rebuildPackage(rec, branch, after, sourceUpdated = after != before)
...
comp.tool(%*{"approval": "always", "timeoutMs": 600000, "onDemand": true}):
  proc plugin_update(package: string): JsonNode =
    ...
    if latest == rec{"ref"}.getStr(""):
      ...
      let commit = repoHead(dir)
      if artifactsNeedRebuild(rec, commit):
        return rebuildPackage(rec, latest, commit, sourceUpdated = false)
      if dir.len > 0 and dirExists(dir):
        try:
