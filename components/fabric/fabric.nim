...
let fabSchema = toolSchema(%*{
  ...
  "maxCalls": {"type": "integer",
   ...
   description = "Write and run a Nim program that drives Niffler tools itself. Use when the work is mechanical and the shape is already known: sequential fan-out over many items, search-then-read distillation, large intermediate results that must never enter the conversation, or retry loops. Sequencing, branching and data flow live in the program; only finish()'s value returns. For exploratory work that needs judgment per step, use the normal loop or agent_run instead.")
