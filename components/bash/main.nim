...
let bashSchema = toolSchema(%*{
  ...
  description = "Execute a shell command via bash -c. Your general-purpose interface to the machine: builds, tests, processes, piping, git mutations, binary inspection. Returns combined stdout/stderr and the exit code of the last command; on timeout the whole process tree is killed (exit 124); when the turn is cancelled mid-command the tree dies promptly (exit 130). Output over 200KB is capped (head + tail kept) with a cut marker — narrow the command (grep/head/tail/wc) rather than assume you saw everything. Prefer dedicated tools where they exist: read/read_many for file content (pageable, caps huge lines), edit/write for changing files, git_* tools (discover on the git component) for status/diff/log/show/blame — approval-free, capped output. Before hand-rolling a network/API integration with curl, check the ecosystem: plugin_search (packages) and skill_list (Agent Skills) may already provide typed tools for it.")
